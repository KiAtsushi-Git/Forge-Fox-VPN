...
- 🔀 Proxy + приложения И сайты одновременно (union mode): каждый поток решается
  по владельцу соединения и адресу назначения, остальное идёт напрямую через
  защищённые сокеты (userspace TCP)
